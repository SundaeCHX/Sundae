...
def signin():
    ...
    images = re.findall(
        r'<font color="red" style="font-size: 12px;">(.*?)<label id="alertp">', content, re.S)
    if images == [u'验证码错误']:
        ...
        items = re.findall(
            r'<div id="login_info2"><span id="term">(.*?)<span id="showOrHide">', content, re.S)
        if items == [u' 2016-2017学年上学期 ']:
            csrftokens = re.findall(
                r'valu.*?action=queryStuLsn&(.*?)></input>', content, re.S)
            csrf = csrftokens[0]
            ...
            score = opener.open('http://210.42.121.132/servlet/Svlt_QueryStuScore?'+csrf+'&year=0&term=&learnType=&scoreFlag=0&t=' +
                                week+'%20'+month+'%20'+date+'%20'+year+'%20'+now+'%20GMT+0800').read().decode('gb2312')
            reports = re.findall(
                r'<tr null>.*?<td>.*?</td>.*?<td>(.*?)</td>.*?<td>(.*?)</td>.*?<td>(.*?)</td>.*?<td>(.*?)</td>.*?<td>(.*?)</td>.*?<td>(.*?)</td>.*?<td>(.*?)</td>.*?<td>(.*?)</td>.*?<td>(.*?)</td>', score, re.S)
            words = ''
            for report in reports:
                if report[8] != '':
                    words = words+u'课程：'+'  ' + \
                        report[0]+'\n'+u'种类：'+report[1]+'    '+u'学分：' + \
                        report[2]+'    '+u'成绩：'+report[8]+'\n\n'
